...
class CustomUserCreationForm(UserCreationForm):
    ...
    def clean_phone(self):
        ...
        if not phone.isdigit():
            raise forms.ValidationError(_("The phone number should only contains digit numbers!"))
        if len(phone) != 11 and len(phone) != 12:
            raise forms.ValidationError(_("The phone number should be 11 or 12 number!"))
        if not (phone.startswith('09') or phone.startswith('98')):
            raise forms.ValidationError(_('The phone number should start with 09 or 98!'))
        return phone
