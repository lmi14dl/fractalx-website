...
def signup_view(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            login(request, user)
            messages.success(request, _("Your account has created"))
...
def login_view(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            user = form.get_user()
            login(request, user)
            messages.success(request, _("You've logged in to your account successfully"))
            return redirect("home")
        else:
...
def logout_view(request):
    logout(request)
    messages.success(request, _("You've logged out from your account!"))
    return redirect('home')
...
def update_user_view(request):
    if request.user.is_authenticated:
        ...
        messages.success(request, _("First you have to login to your account"))
        return redirect('home')
