    ...
    if request.method == 'POST':
        if request.user.is_authenticated:
            try:
                ...
                if not phone:
                    messages.error(request, _("Please add a phone number to your profile"))
                ...
                messages.error(request, _("Please complete your profile with a phone number"))
            ...
            if name and phone:
                ...
                if not phone.isdigit():
                    messages.error(request, _("The phone number should only contain digits!"))
                    return render(request, "services/web_design.html", {'from_post': True})
                elif len(phone) not in [11, 12]:
                    messages.error(request, _("The phone number should be 11 or 12 digits long!"))
                    return render(request, "services/web_design.html", {'from_post': True})
                elif not (phone.startswith('09') or phone.startswith('98')):
                    messages.error(request, _('The phone number should start with 09 or 98!'))
                    return render(request, "services/web_design.html", {'from_post': True})
                else:
                    ContactRequest.objects.create(name=name, phone=phone)
...
def web_design_order_form_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            order.save()
            messages.success(request, _("Your order sent successfully and wait for the response"))
            return render(request, "services/web_design_order_form.html", {'form': form, 'from_post': True})
        else:
            messages.error(request, _('Please check the form errors'))
            return render(request, 'services/web_design_order_form.html', {'form': form, 'from_post': True})
    else:
...
def trading_tutorial_view(request):
    return render(request, "services/trading_tutorial.html", {})